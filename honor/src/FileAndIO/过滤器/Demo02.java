...
//                return pathname.isDirectory() || pathname.toString().toLowerCase().endsWith(
//                        ".java");
//            }
//        });//传递过滤器对象

        //Lamdba
        File[] files = dir.listFiles((pathname -> pathname.isDirectory() || pathname.toString().toLowerCase().endsWith(
                ".java")));//传递过滤器对象
        for (File file : files) {
            if (file.isDirectory()) {
                getAllFiles(file);
